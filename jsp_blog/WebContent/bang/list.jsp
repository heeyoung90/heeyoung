...
<body>
	...
	<FORM name='frm' method='POST' action='./createProc.jsp'
		onsubmit="return Ccheck(this)">
		<TABLE>
			<TR>
				<TH>성명</TH>
				<TD><input type="text" size='10' name="mname"
					value="<%=mname2%>"></TD>
			</TR>
			<TR>
				<TH>내용</TH>
		...
		<TABLE>
			...
			<%
					...
					for (int i = 0; i < list.size(); i++) {
						GbDTO dto = list.get(i);
			%>

			<tr>
				<TH rowspan="4"><img src='./storage/<%=dto.getFname()%>'
					height='70'></TH>
			</tr>
			<TR>
				<Td><%=dto.getGbno()%></Td>
			...
			<TR>
				<TD colspan="3">
					<%
						if (gbno == dto.getGbno()) {
					%> <textarea rows="3" name="content2"><%=dto.getContent()%></textarea>
					<%
						} else {
					%> <%
